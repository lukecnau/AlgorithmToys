...
public final class SHA_256 extends SHA_ABS{
	TOOLS tools = TOOLS.getInstance();
	LOG log = LOG.getInstance();
	
	public final int SHA_256_A_BLOCK_WORDS = 16;
	private final int SHA_256_A_BLOCK_BYTES = SHA_256_A_BLOCK_WORDS*4;
	public final int SHA_256_A_BLOCK_BITS = SHA_256_A_BLOCK_BYTES*8;
	
	public final int SHA_256_A_BLOCK_USED_WORDS = SHA_256_A_BLOCK_WORDS-2;
	public final int SHA_256_A_BLOCK_USED_BYTES = SHA_256_A_BLOCK_USED_WORDS*4;
	public final int SHA_256_A_BLOCK_USED_BITS = SHA_256_A_BLOCK_USED_BYTES*8;
	
	public final int SHA_256_MAIN_LOOP_TIMES = 64;
	
	public final int SHA_256_OUTPUT_BYTES = 32;
	
	private static int[] SHA_256_K = { 
...
//	private final int SHA_256_K_LENS = SHA_256_K.length;
	...
	private int[] paddingMessage(byte[] from) {
		...
		if( null == from || 0 == from.length ) {
			to =  new byte[SHA_256_A_BLOCK_BYTES];
			to[0] = (byte) 0x80;
			
		} else if( from.length < SHA_256_A_BLOCK_USED_BYTES ) {
			to = new byte[SHA_256_A_BLOCK_BYTES];
			...
			to = new byte[(from.length/SHA_256_A_BLOCK_BYTES+(from.length% SHA_256_A_BLOCK_BYTES < SHA_256_A_BLOCK_USED_BYTES ? 1:2))*SHA_256_A_BLOCK_BYTES];
			tools.copyArray(from, 0, from.length, to, 0);
			if( 0 != from.length % SHA_256_A_BLOCK_BYTES )
				to[from.length] = (byte) 0x80;
			tools.longTo8BytesMSB(from.length*8, to, to.length-8);
		}
	...
	public Boolean Digest(byte[] from, byte[] to) {
		...
		int[] W = new int[SHA_256_A_BLOCK_BYTES];
		
		log.printIntegersInHEX("after padding, M", M);
		
		int blocks = M.length / SHA_256_A_BLOCK_WORDS;
		
		System.err.printf("M blocks = %d\n\n", blocks);
		
		int[] H = new int[SHA_256_H0.length];
		for(int i = 0; i < SHA_256_H0.length; ++i)
			H[i] = SHA_256_H0[i];
...
//			System.err.printf("M[%d]=%08X\n", i, M[i]);
		...
		for(int i = 0; i < blocks; ++i) {
			for(int t = 0; t < SHA_256_A_BLOCK_WORDS ; ++t)
				W[t] = M[i * SHA_256_A_BLOCK_WORDS +t];
			
			for(int t = 16; t < SHA_256_MAIN_LOOP_TIMES; ++t)
			...
			int T1, T2;
			for(int t = 0; t < SHA_256_MAIN_LOOP_TIMES; ++t) {
				T1 = h + BSIG1(e) + CH(e, f, g) + SHA_256_K[t] + W[t];
				...
				System.err.printf("t=%02d, a=%08X, b=%08X, c=%08X, d=%08X, e=%08X, f=%08X, g=%08X, h=%08X, W[%02d]=%08X, T1=%08X, T2=%08X\n", t, a, b, c, d, e, f, g, h, t, W[t], T1, T2);
			}
	...
	int getResultBytesSize() {
		return this.SHA_256_OUTPUT_BYTES;
	}
